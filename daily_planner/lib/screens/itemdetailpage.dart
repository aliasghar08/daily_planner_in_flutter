...
class _ItemDetailPageState extends State<ItemDetailPage> {
  ...
  try {
    ...
    final doc = await FirebaseFirestore.instance
        ...
        .get(const GetOptions(source: Source.serverAndCache)); // <-- offline-safe
  ...
  Future<void> _checkNotificationChannel() async {
    ...
    if (channels == null || channels.isEmpty) await _createNotificationChannel();
  }
  ...
  Future<void> _scheduleNativeAndLocalAlarm({
    ...
    try {
      ...
      scaffoldMessengerKey.currentState?.showSnackBar(
        SnackBar(content: Text("Alarm scheduled in ${delay.inSeconds} seconds")),
      );
    } catch (e) {
  ...
  final currentStamps = List<Timestamp>.from(widget.task.completionStamps ?? []);
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                Text(task.detail.isNotEmpty ? task.detail : "No details.", style: const TextStyle(fontSize: 18)),
                const SizedBox(height: 16),
                Row(children: [const Icon(Icons.calendar_today), const SizedBox(width: 8), Text("Date: $formattedDate")]),
                const SizedBox(height: 8),
                Row(children: [const Icon(Icons.access_time), const SizedBox(width: 8), Text("Time: $formattedTime")]),
                const SizedBox(height: 8),
                Row(children: [
                  const Icon(Icons.history),
                  const SizedBox(width: 8),
                  Text("Created on: ${DateFormat('MMM d, yyyy • h:mm a').format(task.createdAt)}")
                ]),
                const SizedBox(height: 16),
                Row(children: [
                  ...
                  GestureDetector(
                    ...
                    child: Chip(
                      label: Text(_currentCompletionStatus! ? "Completed" : "Pending"),
                ...
                if (task.taskType != 'oneTime') ...[_buildCompletionTimesExpansion()],
  ...
  Widget _buildCompletionTimesExpansion() => ExpansionTile(
        ...
        children: completedList.isEmpty
            ? [const Padding(
                ...
                child: Center(child: Text("No completion times available", style: TextStyle(color: Colors.grey))),
              )]
            : completedList
                .map((date) => ListTile(
  ...
  Widget _buildNotificationTimesExpansion() => ExpansionTile(
        ...
        children: notificationTimes.isEmpty
            ? [const Padding(
                padding: EdgeInsets.symmetric(vertical: 12),
                child: Center(child: Text("No Notification Times available", style: TextStyle(color: Colors.grey))),
              )]
            : notificationTimes
                .map((date) => ListTile(
                      leading: const Icon(Icons.check),
  ...
  Widget _buildNotificationTestButtons() => Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text("🔔 Notification Test Buttons", style: TextStyle(fontWeight: FontWeight.bold)),
          const Padding(
          ...
          ElevatedButton(onPressed: _triggerTestAlarm, child: const Text("Test Alarm (2 seconds)")),
          const SizedBox(height: 8),
          ElevatedButton(
              onPressed: () => _scheduleNativeAndLocalAlarm(
          ...
          if (widget.task.editHistory.isEmpty)
            ...
            Column(
              ...
              children: widget.task.editHistory.map((edit) {
                final formattedEditTime = DateFormat('MMM d, yyyy • h:mm a').format(edit.timestamp);
                return ListTile(
                  leading: const Icon(Icons.edit_note),
                  title: Text(formattedEditTime),
                  subtitle: edit.note != null && edit.note!.isNotEmpty ? Text(edit.note!) : const Text("No note"),
